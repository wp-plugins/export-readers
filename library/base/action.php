<?php
class bv39v_action extends bv39v_base {
	...
	protected function set_view() {
		if (null === $this->view) {
			$this->view = new bv39v_view ( $this->application () );
		}
	}
	...
	public function updated($message = null,$force=false) {
		...
		if ($this->request()->is_post() || $force) {
			...
			$return = $this->render_script ( 'common/notices.phtml' );
		}
		return $return;
	}
	protected function marker($tag, $content) {
		$tagc = bv39v_tag::instance ();
	...
	protected function add_action_type($action_type, $tag = null, $default_meta = array()) {
		...
		$meta ['probono'] = false;
		$meta = bv39v_data_array::merge ( $meta, $default_meta );
		$meta ['tag'] = $tag;
