...
class b6v_Settings extends b6v_Controller_Action {
	...
	public function get_full($html = false) {
		...
		foreach ( $this->settings ['settings_folders'] as $folder ) {
			...
			while ( false !== ($entry = $d->read ()) ) {
				$name = $d->path . $entry;
				if (fnmatch ( '*.phtml', $entry )) {
					$pi = pathinfo ( $name );
					$fname = $pi ['filename'];
					$index = $fname;
					if (fnmatch ( '*_ext.phtml', $entry )) {
