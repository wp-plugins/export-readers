<?php
class usersexportreaders extends wv39v_action {
...
HAVING
	count(*) >= %d
";
		$comments = new wv39v_data_table ( 'comments' );
...
WHERE
	`user_registered` >= '%s'
";
		$users = new wv39v_data_table ( 'users' );
		$usermeta = new wv39v_data_table ( 'usermeta' );
		global $table_prefix;
		$sql = sprintf ( $sql, $users->name (), $usermeta->name (),$table_prefix, $minDate );
