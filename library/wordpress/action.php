<?php
class wv39v_action extends bv39v_action {
	...
	public function test_data() {
		$posts = $this->application ()->data ( '' )->posts;
	...
	protected function set_view() {
		$this->view = new wv39v_view ( $this->application () );
	}
	...
	public function callback() {
		...
		if ($menu) {
			...
			{
				$this->view->form_name = 'v39v_form';
			}
			$output = "<form  id='{$this->view->form_name}' action='' method='post' class='admin-menu {$classes}'>{$updated}{$output}</form>";
	...
	public function wrapper($page, $classes = array(), $attr = null, $tag = 'div') {
		$classes [] = 'wrap';
		$classes [] = 'v39v';
		$classes [] = 'v39v_' . $this->application ()->slug;
		...
		foreach ( $this->view->items as $key => $value ) {
			if ($value ['hide']) {
				...
				if ((! isset ( $_GET ['page2'] ) && ! $current) || substr ( $_SERVER ['REQUEST_URI'], - strlen ( $this->view->items [$key] ['url'] ) ) == $this->view->items [$key] ['url']) {
					$this->view->items [$key] ['classes'] [] = 'v39v_current';
					$current = true;
				}
