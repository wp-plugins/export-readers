<?php
class bv39v_data_settings extends bv39v_base {
	protected $_option;
	...
	private static function load($file, $legacy = false) {
		...
		if (file_exists ( $file )) {
			switch (pathinfo ( $file, PATHINFO_EXTENSION )) {
				case 'xml' :
					$return = bv39v_data_xml::load ( $file );
					break;
				case 'json' :
					$return = bv39v_data_json::decode ( file_get_contents ( $file ), $legacy );
					break;
			}
		}
		return $return;
	...
	public function refresh() {
		...
		if (null !== $this->_option) {
			$base = new bv39v_data_settings ( $this->application () );
			$settings [] = $base->data ();
		...
		$this->_data = bv39v_data_array::merge ( $settings );
		return $this->_data;
	}
