<?php
abstract class w6v_Controller_Action_Abstract extends b6v_Controller_Action
{
	...
	{
		$this->view = new w6v_View ( $this->application () );
	}
	...
	protected function selected_action_wp()
	{
		...
		if (count ( $filter ) > 1 && $filter [1] == 'page')
		{
			...
			else
			{
				foreach ( $this->actions () as $r )
				{
					if ($pages ['page2'] == b6v_Type_String::staticSafe ( $r ['title'] ))
					{return $r;}
				}
			}
		}
	...
	protected function subpages ()
	{
		...
		{
			if (b6v_Type_String::staticStartsWith ( $key , 'page' ))
			{
				$pages [$key] = $value;
			}
		}
