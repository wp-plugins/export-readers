<?php
abstract class b6v_Controller_Action extends b6v_Base {
	protected function marker($tag, $content) {
		$tagc = b6v_Tag::instance ();
		$matches = $tagc->get ( $tag, $content, true );
		foreach ( ( array ) $matches as $match ) {
			$new = call_user_func ( array ($this, $tag . '_Marker' ), $match );
			$content = str_replace ( $match ['match'], w6v_Values::fixPostInsert ( $new ), $content );
		}
		return $content;
	}
	private $_type = null;
	...
	protected function setView() {
		if (null === $this->view) {
			$this->view = new b6v_View ( $this->application () );
		}
	}
	...
	protected function renderScript($script, $absPath = null) {
		$return = null;
		$orig = $script;
		if (null !== $absPath) {
			throw new exception ( 'abspath is redundant, remove it' );
		}
		...
		if ($script !== false) {
			...
			switch (strtolower ( $pi ['extension'] )) {
				case 'ini' :
					$data = new b6v_Data_INI ( $this->application (), $script );
					$return = $data->load ();
					$return = $return->getArray ();
					break;
				case 'csv' :
					$data = new b6v_Data_CSV ( $this->application (), $script, true );
	...
	public function loadTemplate($filename) {
		throw new exception ( 'use loadTemplate instead of renderScript' );
	}
