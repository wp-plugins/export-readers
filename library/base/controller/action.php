<?php
abstract class bv19v_controller_action extends bv19v_base {
	protected function marker($tag, $content) {
		$tagc = bv19v_tag::instance ();
	...
	protected function set_view() {
		if (null === $this->view) {
			$this->view = new bv19v_view ( $this->application () );
		}
	}
	...
	protected function render_script($script, $html = true) {
		...
		if ($string === false) {
			...
			if ($script !== false) {
				switch ($pi ['extension']) {
					case 'xml' :
						$data = new bv19v_data_xml ( $this->application (), $script, true );
						$return = $data->load ();
						$html = false;
						break;
					default :
